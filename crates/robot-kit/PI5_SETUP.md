...
Complete guide to setting up a clawclawclaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.clawclawclaw/models
cp models/ggml-base.bin ~/.clawclawclaw/models/
```
...
mkdir -p ~/.clawclawclaw/models/piper
cd ~/.clawclawclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.clawclawclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build clawclawclaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/CSZHK/clawcalwclaw
cd clawclawclaw

# Build robot kit
cargo build --release -p clawclawclaw-robot-kit

# Build main clawclawclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.clawclawclaw
nano ~/.clawclawclaw/robot.toml
```

```toml
# ~/.clawclawclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.clawclawclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/clawclawclaw_sensors.fifo"
...
### Start clawclawclaw Agent

```bash
# Configure clawclawclaw to use robot tools
cat > ~/.clawclawclaw/config.toml << 'EOF'
...
cp ~/clawclawclaw/crates/robot-kit/SOUL.md ~/.clawclawclaw/workspace/

# Start agent
./target/release/clawclawclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/clawclawclaw_sensors.fifo ]; then
    mkfifo /tmp/clawclawclaw_sensors.fifo
fi
...
# Start clawclawclaw
cd ~/clawclawclaw
./target/release/clawclawclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/clawclawclaw-robot.service
sudo tee /etc/systemd/system/clawclawclaw-robot.service << 'EOF'
[Unit]
Description=clawclawclaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/clawclawclaw
...
sudo systemctl enable clawclawclaw-robot
sudo systemctl start clawclawclaw-robot

# Check status
sudo systemctl status clawclawclaw-robot
journalctl -u clawclawclaw-robot -f  # View logs
```
