# clawclawclaw Robot Kit

A complete toolkit for building AI-powered robots with clawclawclaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build clawclawclaw with robot tools

```bash
# Clone and build
git clone https://github.com/CSZHK/clawcalwclaw
cd clawclawclaw
cargo build -p clawclawclaw-robot-kit --release
```
...
cp crates/robot-kit/robot.toml ~/.clawclawclaw/
cp crates/robot-kit/SOUL.md ~/.clawclawclaw/workspace/
...
# Test in mock mode
./target/release/clawclawclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/clawclawclaw agent -m "Move forward 1 meter"
```
...
```rust
use clawclawclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `clawclawclaw`, add a thin adapter that maps this
...
mkdir clawclawclaw-robot-kit
cp -r target/release/clawclawclaw clawclawclaw-robot-kit/
cp -r examples/robot_kit clawclawclaw-robot-kit/
cp -r ~/.clawclawclaw clawclawclaw-robot-kit/dot-clawclawclaw

# Include models
mkdir -p clawclawclaw-robot-kit/models
cp ~/.clawclawclaw/models/ggml-base.bin clawclawclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf clawclawclaw-robot-kit.tar.gz clawclawclaw-robot-kit/

# Copy to USB
cp clawclawclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
