# clawclawclaw Development Environment

A fully containerized development sandbox for clawclawclaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`clawclawclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `clawclawclaw` CLI commands manually, debug the binary, or check logs internally.

- **Path**: `/clawclawclaw-data`
- **User**: `nobody` (65534)
...
- **Agent**: `/clawclawclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.clawclawclaw` (mounted to `/clawclawclaw-data/.clawclawclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.clawclawclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
```bash
cp dev/config.wasm.staging.toml target/.clawclawclaw/config.toml
```
...
- Rust compilation, tests, and audit/deny tools run in `clawclawclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=clawclawclaw-target`) to speed repeat local image builds.
