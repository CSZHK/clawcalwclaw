# clawclawclaw Plugin System

A plugin architecture for clawclawclaw modeled after [OpenClaw's plugin system](https://github.com/openclaw/openclaw), adapted for Rust.

## Overview

The plugin system allows extending clawclawclaw with custom tools, hooks, channels, and providers without modifying the core codebase. Plugins are discovered from standard directories, loaded at startup, and registered with the host through a clean API.

## Architecture

### Key Components

1. **Manifest** (`clawclawclaw.plugin.toml`): Declares plugin metadata (id, name, version, description)
...
| OpenClaw (TypeScript)              | clawclawclaw (Rust)                    |
|------------------------------------|------------------------------------|
| `openclaw.plugin.json`             | `clawclawclaw.plugin.toml`             |
...
`extensions/hello-world/clawclawclaw.plugin.toml`:

```toml
id = "hello-world"
name = "Hello World"
description = "Example plugin demonstrating the clawclawclaw plugin API."
version = "0.1.0"
```
...
use clawclawclaw::tools::traits::{Tool, ToolResult};
...
#[async_trait]
impl clawclawclaw::hooks::HookHandler for HelloHook {
...
```rust
use clawclawclaw::plugins::{load_plugins, Plugin};
...
2. **Global**: `~/.clawclawclaw/extensions/`
3. **Workspace**: `<workspace>/.clawclawclaw/extensions/`
4. **Custom**: Paths in `plugins.load_paths`

Each directory is scanned for subdirectories containing `clawclawclaw.plugin.toml`.
...
- A bad plugin won't crash clawclawclaw
...
Implement `clawclawclaw::hooks::HookHandler`:
...
- **Hot reload**: Reload plugins without restarting clawclawclaw
...
- [Issue #1414](https://github.com/CSZHK/clawcalwclaw/issues/1414)
