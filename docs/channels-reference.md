...
This document is the canonical reference for channel configuration in clawclawclaw.
...
6. **Stale runtime state**: config changed but `clawclawclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.clawclawclaw/config.toml`.
...
One clawclawclaw runtime can serve multiple channels at once: if you configure several
channel sub-tables, `clawclawclaw channel start` launches all of them in the same process.
...
When running `clawclawclaw channel start` (or daemon mode), runtime commands include:
...
- Model cache previews come from `clawclawclaw models refresh --provider <ID>`.
...
clawclawclaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `clawclawclaw channel list`, `clawclawclaw channel doctor`, and `clawclawclaw channel start` will report that the channel is intentionally skipped for this build. The same applies to cron delivery: setting `delivery.channel` to a feature-gated channel in a build without that feature will return an error at delivery time. For Matrix cron delivery, only plain rooms are supported; E2EE rooms require listener sessions via `clawclawclaw daemon`.
...
access_token = "syt_..."
user_id = "@clawclawclaw:matrix.example.com"   # optional, recommended for E2EE
...
clawclawclaw supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.clawclawclaw/state/whatsapp-web/session.db"
...
allowed_senders = ["*"]
imap_id = { enabled = true, name = "clawclawclaw", version = "0.1.7", vendor = "clawclawclaw-labs" }
```
...
port = 6697
nickname = "clawclawclaw-bot"
username = "clawclawclaw"              # optional
channels = ["#clawclawclaw"]
allowed_users = ["*"]
...
- If image download fails, clawclawclaw forwards fallback text instead of silently dropping the message.
...
- `CLAWCLAWCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `CLAWCLAWCLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
- `CLAWCLAWCLAW_WATI_WEBHOOK_SECRET` overrides `webhook_secret` when set.

### 4.21 ACP

ACP (Agent Client Protocol) enables clawclawclaw to act as a client for OpenCode ACP server,
...
5. Restart `clawclawclaw daemon` after config changes.
...
```bash
RUST_LOG=info clawclawclaw daemon 2>&1 | tee /tmp/clawclawclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel|ACP" /tmp/clawclawclaw.log
```
