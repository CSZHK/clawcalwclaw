...
This document describes the production binary build lane for clawclawclaw on Blacksmith-backed GitHub Actions runners.
...
- Binary path: `target/release/clawclawclaw`
- Uploaded artifact name: `clawclawclaw-linux-amd64`
- Uploaded files:
    - `artifacts/clawclawclaw`
    - `artifacts/clawclawclaw.sha256`
...
4. Download `clawclawclaw-linux-amd64` from the run artifacts and verify checksum:

```bash
sha256sum -c clawclawclaw.sha256
```
