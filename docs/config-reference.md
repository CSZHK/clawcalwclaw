# clawclawclaw Config Reference (Operator-Oriented)
...
1. `CLAWCLAWCLAW_WORKSPACE` override (if set)
2. persisted `~/.clawclawclaw/active_workspace.toml` marker (if present)
3. default `~/.clawclawclaw/config.toml`

clawclawclaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

CLI commands for config inspection and modification:

- `clawclawclaw config show` — print effective config as JSON (secrets masked)
- `clawclawclaw config get <key>` — query a value by dot-path (e.g. `clawclawclaw config get gateway.port`)
- `clawclawclaw config set <key> <value>` — update a value and save to `config.toml`
- `clawclawclaw config schema` — print JSON Schema (draft 2020-12) to stdout
...
- Environment override: `CLAWCLAWCLAW_MODEL_SUPPORT_VISION` or `MODEL_SUPPORT_VISION` (values: `true`/`false`/`1`/`0`/`yes`/`no`/`on`/`off`).
...
| `otel_service_name` | `clawclawclaw` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `clawclawclaw doctor traces --limit 20`
  - `clawclawclaw doctor traces --event tool_call_result --contains \"error\"`
  - `clawclawclaw doctor traces --id <trace-id>`
...
otel_service_name = "clawclawclaw"
...
1. `CLAWCLAWCLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `CLAWCLAWCLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
- For OpenAI-compatible Responses fallback transport:
  - `CLAWCLAWCLAW_RESPONSES_WEBSOCKET=1` forces websocket-first mode (`wss://.../responses`) for compatible providers.
  - `CLAWCLAWCLAW_RESPONSES_WEBSOCKET=0` forces HTTP-only mode.
...
- When `enabled = true` and no OTP secret exists, clawclawclaw generates one and prints an enrollment URI once.
...
| `state_file` | `~/.clawclawclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `clawclawclaw estop` to engage and `clawclawclaw estop resume` to clear levels.
...
- `CLAWCLAWCLAW_URL_ACCESS_BLOCK_PRIVATE_IP` / `URL_ACCESS_BLOCK_PRIVATE_IP`
- `CLAWCLAWCLAW_URL_ACCESS_ALLOW_LOOPBACK` / `URL_ACCESS_ALLOW_LOOPBACK`
- `CLAWCLAWCLAW_URL_ACCESS_REQUIRE_FIRST_VISIT_APPROVAL` / `URL_ACCESS_REQUIRE_FIRST_VISIT_APPROVAL`
- `CLAWCLAWCLAW_URL_ACCESS_ENFORCE_DOMAIN_ALLOWLIST` / `URL_ACCESS_ENFORCE_DOMAIN_ALLOWLIST`
- `CLAWCLAWCLAW_URL_ACCESS_ALLOW_CIDRS` / `URL_ACCESS_ALLOW_CIDRS` (comma-separated)
- `CLAWCLAWCLAW_URL_ACCESS_ALLOW_DOMAINS` / `URL_ACCESS_ALLOW_DOMAINS` (comma-separated)
- `CLAWCLAWCLAW_URL_ACCESS_DOMAIN_ALLOWLIST` / `URL_ACCESS_DOMAIN_ALLOWLIST` (comma-separated)
- `CLAWCLAWCLAW_URL_ACCESS_DOMAIN_BLOCKLIST` / `URL_ACCESS_DOMAIN_BLOCKLIST` (comma-separated)
- `CLAWCLAWCLAW_URL_ACCESS_APPROVED_DOMAINS` / `URL_ACCESS_APPROVED_DOMAINS` (comma-separated)
...
- When set, overrides `CLAWCLAWCLAW_CODEX_REASONING_EFFORT` for OpenAI Codex requests.
- Unset falls back to `CLAWCLAWCLAW_CODEX_REASONING_EFFORT` if present, otherwise defaults to `xhigh`.
...
- Transport override precedence for OpenAI Codex:
  ...
  2. `PROVIDER_TRANSPORT` / `CLAWCLAWCLAW_PROVIDER_TRANSPORT` / `CLAWCLAWCLAW_CODEX_TRANSPORT`
  3. `provider.transport`
  4. legacy `CLAWCLAWCLAW_RESPONSES_WEBSOCKET` (boolean)
...
- Security-first default: clawclawclaw does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `CLAWCLAWCLAW_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `CLAWCLAWCLAW_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `CLAWCLAWCLAW_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `CLAWCLAWCLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- `clawclawclaw skills install` and `clawclawclaw skills audit` apply a static security audit. Skills that contain script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- clawclawclaw requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
| `user_agent` | `clawclawclaw/1.0` | User-Agent header for outbound HTTP requests |
...
| `timeout_secs` | `30` | Request timeout in seconds |
| `user_agent` | `clawclawclaw/1.0` | User-Agent header for fetch requests |
...
| `user_agent` | `clawclawclaw/1.0` | User-Agent header for search requests |
...
- `CLAWCLAWCLAW_WEB_SEARCH_ENABLED` / `WEB_SEARCH_ENABLED`
- `CLAWCLAWCLAW_WEB_SEARCH_PROVIDER` / `WEB_SEARCH_PROVIDER`
- `CLAWCLAWCLAW_WEB_SEARCH_MAX_RESULTS` / `WEB_SEARCH_MAX_RESULTS`
- `CLAWCLAWCLAW_WEB_SEARCH_TIMEOUT_SECS` / `WEB_SEARCH_TIMEOUT_SECS`
- `CLAWCLAWCLAW_WEB_SEARCH_FALLBACK_PROVIDERS` / `WEB_SEARCH_FALLBACK_PROVIDERS` (comma-separated)
- `CLAWCLAWCLAW_WEB_SEARCH_RETRIES_PER_PROVIDER` / `WEB_SEARCH_RETRIES_PER_PROVIDER`
- `CLAWCLAWCLAW_WEB_SEARCH_RETRY_BACKOFF_MS` / `WEB_SEARCH_RETRY_BACKOFF_MS`
- `CLAWCLAWCLAW_WEB_SEARCH_DOMAIN_FILTER` / `WEB_SEARCH_DOMAIN_FILTER` (comma-separated)
- `CLAWCLAWCLAW_WEB_SEARCH_LANGUAGE_FILTER` / `WEB_SEARCH_LANGUAGE_FILTER` (comma-separated)
- `CLAWCLAWCLAW_WEB_SEARCH_COUNTRY` / `WEB_SEARCH_COUNTRY`
- `CLAWCLAWCLAW_WEB_SEARCH_RECENCY_FILTER` / `WEB_SEARCH_RECENCY_FILTER`
- `CLAWCLAWCLAW_WEB_SEARCH_MAX_TOKENS` / `WEB_SEARCH_MAX_TOKENS`
- `CLAWCLAWCLAW_WEB_SEARCH_MAX_TOKENS_PER_PAGE` / `WEB_SEARCH_MAX_TOKENS_PER_PAGE`
- `CLAWCLAWCLAW_WEB_SEARCH_EXA_SEARCH_TYPE` / `WEB_SEARCH_EXA_SEARCH_TYPE`
- `CLAWCLAWCLAW_WEB_SEARCH_EXA_INCLUDE_TEXT` / `WEB_SEARCH_EXA_INCLUDE_TEXT`
- `CLAWCLAWCLAW_WEB_SEARCH_JINA_SITE_FILTERS` / `WEB_SEARCH_JINA_SITE_FILTERS` (comma-separated)
- `CLAWCLAWCLAW_BRAVE_API_KEY` / `BRAVE_API_KEY`
- `CLAWCLAWCLAW_PERPLEXITY_API_KEY` / `PERPLEXITY_API_KEY`
- `CLAWCLAWCLAW_EXA_API_KEY` / `EXA_API_KEY`
- `CLAWCLAWCLAW_JINA_API_KEY` / `JINA_API_KEY`
...
3. Validate with `clawclawclaw doctor` before restart/rollout.
...
- While `clawclawclaw channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `CLAWCLAWCLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `CLAWCLAWCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
...
Inter-process communication for independent clawclawclaw agents on the same host.

| Key | Default | Purpose |
|---|---|---|
| `enabled` | `false` | Enable IPC tools (`agents_list`, `agents_send`, `agents_inbox`, `state_get`, `state_set`) |
| `db_path` | `~/.clawclawclaw/agents.db` | Shared SQLite database path (all agents on this host share one file) |
...
[agents_ipc]
enabled = true
db_path = "~/.clawclawclaw/agents.db"
staleness_secs = 300
```
