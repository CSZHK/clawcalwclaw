...
| Scenario | Command |
|----------|---------|
| I have an API key, want fastest setup | `clawclawclaw onboard --api-key sk-... --provider openrouter` |
| I want guided prompts | `clawclawclaw onboard --interactive` |
| Config exists, just fix channels | `clawclawclaw onboard --channels-only` |
| Config exists, I intentionally want full overwrite | `clawclawclaw onboard --force` |
| Using OpenAI Codex subscription auth | See [OpenAI Codex OAuth Quick Setup](#openai-codex-oauth-quick-setup) |

## Onboarding and Validation

- Quick onboarding: `clawclawclaw onboard --api-key "sk-..." --provider openrouter`
- Interactive onboarding: `clawclawclaw onboard --interactive`
...
- Validate environment: `clawclawclaw status` + `clawclawclaw doctor`
...
```bash
clawclawclaw auth login --provider openai-codex
```

2. Verify auth material is loaded:

```bash
clawclawclaw auth status --provider openai-codex
```
