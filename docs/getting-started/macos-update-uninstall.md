...
This page documents supported update and uninstall procedures for clawclawclaw on macOS (OS X).
...
- Homebrew: `/opt/homebrew/bin/clawclawclaw` (Apple Silicon) or `/usr/local/bin/clawclawclaw` (Intel)
- Cargo/bootstrap/manual: `~/.cargo/bin/clawclawclaw`
...
```bash
clawclawclaw update --instructions
clawclawclaw update --check
```
