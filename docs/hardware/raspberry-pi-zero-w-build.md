# Building clawclawclaw on Raspberry Pi Zero W

Complete guide to compile clawclawclaw on Raspberry Pi Zero W (512MB RAM, ARMv6).
...
- Some niche dependencies may not support musl (clawclawclaw's dependencies are musl-compatible)
...
### Step 5: Clone clawclawclaw Repository
...
clawclawclaw's `Cargo.toml` is already configured for low-memory devices (`codegen-units = 1` in release profile). For additional safety on Pi Zero W:
...
### Step 7: Choose Target ABI and Build clawclawclaw
...
# For musleabihf
sudo cp target/armv6l-unknown-linux-musleabihf/release/clawclawclaw /usr/local/bin/

# Verify installation
clawclawclaw --version

# Verify binary is statically linked (musleabihf only)
file /usr/local/bin/clawclawclaw
...
The clawclawclaw repository includes pre-configured `.cargo/config.toml` and `.cargo/armv6l-unknown-linux-musleabihf.json` for static linking.

```bash
# Clone clawclawclaw repository
git clone https://github.com/CSZHK/clawcalwclaw.git
cd clawclawclaw
...
```bash
file target/armv6l-unknown-linux-musleabihf/release/clawclawclaw
# Output should include: "statically linked"

ldd target/armv6l-unknown-linux-musleabihf/release/clawclawclaw
...
scp target/armv6l-unknown-linux-musleabihf/release/clawclawclaw pi@zero-w-ip:/home/pi/

# On Pi Zero W
sudo mv ~/clawclawclaw /usr/local/bin/
sudo chmod +x /usr/local/bin/clawclawclaw
clawclawclaw --version

# Verify it's statically linked (no dependencies on target system)
ldd /usr/local/bin/clawclawclaw
...
```
┌─────────────────┐     Clone/Fork     ┌─────────────────────┐
│  clawclawclaw Repo  │ ──────────────────> │   Your Build Host   │
...
mkdir -p ~/.config/clawclawclaw
nano ~/.config/clawclawclaw/config.toml
```
...
```bash
sudo nano /etc/systemd/system/clawclawclaw.service
```
...
```ini
[Unit]
Description=clawclawclaw AI Agent
...
ExecStart=/usr/local/bin/clawclawclaw agent
Restart=on-failure
...
sudo systemctl enable clawclawclaw
sudo systemctl start clawclawclaw
```
