# Οδηγός Ρυθμίσεων clawclawclaw (config.toml)
...
Το clawclawclaw ψάχνει για τις ρυθμίσεις με την εξής σειρά:
1. Στη διαδρομή που ορίζει η μεταβλητή `CLAWCLAWCLAW_WORKSPACE`.
2. Στο αρχείο `~/.clawclawclaw/config.toml` (αυτή είναι η συνηθισμένη θέση).
...
- Αν αλλάξετε το αρχείο `config.toml`, πρέπει να κάνετε επανεκκίνηση το clawclawclaw για να δει τις αλλαγές.
- Χρησιμοποιήστε την εντολή `clawclawclaw doctor` για να βεβαιωθείτε ότι οι ρυθμίσεις σας είναι σωστές.
