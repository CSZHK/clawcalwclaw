# Ανάπτυξη Δικτύου — clawclawclaw σε Raspberry Pi και Τοπικό Δίκτυο

Αυτό το έγγραφο καλύπτει την ανάπτυξη του clawclawclaw σε ένα Raspberry Pi ή σε άλλον κεντρικό υπολογιστή στο τοπικό σας δίκτυο, με κανάλια Telegram και προαιρετικά κανάλια webhook.
...
|------|----------------------|----------|
| **Telegram polling** | Όχι | Το clawclawclaw αντλεί δεδομένα από το API του Telegram. Λειτουργεί από παντού. |
| **Matrix sync (συμπ. E2EE)** | Όχι | Το clawclawclaw συγχρονίζεται μέσω του API του Matrix. Δεν απαιτείται εισερχόμενο webhook. |
...
**Σημείωση:** Τα Telegram, Discord, Slack και Nostr χρησιμοποιούν **εξερχόμενες συνδέσεις** — το clawclawclaw συνδέεται σε εξωτερικούς διακομιστές. Δεν απαιτείται προώθηση θυρών (port forwarding) ή δημόσια IP.

---

## 2. clawclawclaw σε Raspberry Pi
...
Επεξεργαστείτε το αρχείο `~/.clawclawclaw/config.toml`:
...
- Το κανάλι Telegram λειτουργεί: το clawclawclaw αντλεί δεδομένα από το API του Telegram (εξερχόμενη σύνδεση).
...
2. Ξεκινήστε μια σήραγγα (tunnel):
   ...
   ```
   Ή χρησιμοποιήστε την εντολή `clawclawclaw tunnel`.

3. Το clawclawclaw θα απορρίψει το `0.0.0.0` εκτός εάν η επιλογή `allow_public_bind = true` ή μια σήραγγα είναι ενεργή.
...
- Το clawclawclaw καλεί το `https://api.telegram.org/bot{token}/getUpdates`.
...
Εκτελέστε το `clawclawclaw daemon` — το κανάλι Telegram ξεκινά αυτόματα.

Για την έγκριση ενός λογαριασμού Telegram κατά την εκτέλεση:

```bash
clawclawclaw channel bind-telegram <ΤΑΥΤΟΤΗΤΑ>
```
...
- Διατηρήστε μόνο μία ενεργή εκτέλεση για το ίδιο token (συνιστάται: η υπηρεσία `clawclawclaw daemon`).
...
- [ ] Εκτέλεση `clawclawclaw daemon --host 127.0.0.1 --port 42617`.
...
Το clawclawclaw υποστηρίζει το OpenRC για το Alpine Linux και άλλες διανομές που χρησιμοποιούν το σύστημα αρχικοποίησης OpenRC. Οι υπηρεσίες OpenRC εκτελούνται **σε όλο το σύστημα** και απαιτούν δικαιώματα root/sudo.

### 7.1 Προαπαιτούμενα

- Alpine Linux (ή άλλη διανομή βασισμένη στο OpenRC).
- Πρόσβαση Root ή sudo.
- Ένας αποκλειστικός χρήστης συστήματος `clawclawclaw` (δημιουργείται κατά την εγκατάσταση).
...
Αυτό δημιουργεί:
- Σενάριο αρχικοποίησης (Init script): `/etc/init.d/clawclawclaw`
- Κατάλογο ρυθμίσεων: `/etc/clawclawclaw/`
- Κατάλογο καταγραφών (Logs): `/var/log/clawclawclaw/`

### 7.3 Ρύθμιση

Συνήθως δεν απαιτείται χειροκίνητη αντιγραφή των ρυθμίσεων. Η εντολή `sudo clawclawclaw service install` προετοιμάζει αυτόματα το `/etc/clawclawclaw`, μεταφέρει την υπάρχουσα κατάσταση από τις ρυθμίσεις του χρήστη σας και ορίζει τις σωστές άδειες για τον χρήστη της υπηρεσίας `clawclawclaw`.
...
| Εντολή | Περιγραφή |
|---------|-------------|
| `sudo rc-service clawclawclaw start` | Έναρξη του δαίμονα |
| `sudo rc-service clawclawclaw stop` | Διακοπή του δαίμονα |
| `sudo rc-service clawclawclaw status` | Έλεγχος κατάστασης υπηρεσίας |
| `sudo rc-service clawclawclaw restart` | Επανεκκίνηση του δαίμονα |
...
| Καταγραφή | Διαδρομή |
|-----|------|
| Πρόσβαση/stdout | `/var/log/clawclawclaw/access.log` |
| Σφάλματα/stderr | `/var/log/clawclawclaw/error.log` |
