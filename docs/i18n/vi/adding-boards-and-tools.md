# Thêm Board và Tool — Hướng dẫn phần cứng clawclawclaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào clawclawclaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.clawclawclaw/config.toml)
clawclawclaw peripheral add nucleo-f401re /dev/ttyACM0
clawclawclaw peripheral add arduino-uno /dev/cu.usbmodem12345
clawclawclaw peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, clawclawclaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `clawclawclaw peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `clawclawclaw peripheral list` | Liệt kê các board đã cấu hình |
| `clawclawclaw peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `clawclawclaw peripheral flash` | Nạp firmware Arduino |
| `clawclawclaw peripheral flash-nucleo` | Nạp firmware Nucleo |
| `clawclawclaw hardware discover` | Liệt kê thiết bị USB |
| `clawclawclaw hardware info` | Thông tin chip qua probe-rs |
