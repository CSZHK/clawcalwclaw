...
[INFO] ✓ Audit logging enabled (~/.config/clawclawclaw/audit.log)
...
```toml
# ~/.config/clawclawclaw/config.toml
...
$ clawclawclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/clawclawclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ clawclawclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ clawclawclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ clawclawclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ clawclawclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `clawclawclaw security --status`
