# Triển khai mạng — clawclawclaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai clawclawclaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | clawclawclaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | clawclawclaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — clawclawclaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. clawclawclaw trên Raspberry Pi
...
Chỉnh sửa `~/.clawclawclaw/config.toml`:
...
- Channel Telegram hoạt động: clawclawclaw poll Telegram API (outbound)
...
```bash
clawclawclaw daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `clawclawclaw tunnel` (xem tài liệu tunnel).

3. clawclawclaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- clawclawclaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `clawclawclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `clawclawclaw daemon`).
...
- [ ] Chạy `clawclawclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
