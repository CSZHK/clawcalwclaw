# Tài liệu tham khảo Providers — clawclawclaw
...
3. Biến môi trường dự phòng chung: `CLAWCLAWCLAW_API_KEY`, sau đó là `API_KEY`
...
- Sau khi chuẩn hóa multimodal, clawclawclaw gửi payload hình ảnh qua trường `messages[].images` gốc của Ollama.
- Nếu chọn provider không hỗ trợ vision, clawclawclaw trả về lỗi rõ ràng thay vì âm thầm bỏ qua hình ảnh.
...
Vì clawclawclaw không thể tự động phát hiện, bạn có thể ghi đè trong `config.toml`:
...
Biến môi trường: `CLAWCLAWCLAW_MODEL_SUPPORT_VISION=true`
...
- Khi đặt, ghi đè `CLAWCLAWCLAW_CODEX_REASONING_EFFORT`.
- Không đặt sẽ dùng `CLAWCLAWCLAW_CODEX_REASONING_EFFORT` nếu có, nếu không mặc định `xhigh`.
...
- Khám phá model: `clawclawclaw models refresh --provider nvidia`
