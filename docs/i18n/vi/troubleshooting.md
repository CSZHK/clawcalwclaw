...
- `cargo check` / `cargo build` dừng lâu ở `Checking clawclawclaw`
...
### Không tìm thấy lệnh `clawclawclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `clawclawclaw`
...
Xác minh `~/.clawclawclaw/config.toml`:
...
- Dừng các tiến trình `clawclawclaw daemon` / `clawclawclaw channel start` thừa
