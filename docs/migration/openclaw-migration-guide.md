# OpenClaw → clawclawclaw Migration Guide

This guide walks you through migrating an OpenClaw deployment to clawclawclaw. It covers configuration conversion, endpoint changes, and the architectural differences you need to know.

## Quick Start (Built-in Merge Migration)

clawclawclaw now includes a built-in OpenClaw migration flow:
...
Localization status: this guide currently ships in English only. Localized follow-through for `zh-CN`, `ja`, `ru`, `fr`, `vi`, and `el` is deferred; translators should carry over the exact CLI forms `clawclawclaw migrate openclaw` and `clawclawclaw onboard --migrate-openclaw` first.

Default migration semantics are **merge-first**:

- Existing clawclawclaw values are preserved (no blind overwrite).
...
# 2. The compatibility layer is built into clawclawclaw — no files to copy.
...
clawclawclaw is a **standalone messaging gateway**. It owns the full agent loop internally. Channels (WhatsApp, Linq, Nextcloud Talk) send a single message string, and clawclawclaw handles everything: system prompt construction, tool invocation, memory recall, context enrichment, and response generation.
...
|----------|--------|------------|----------|
| `POST /api/chat` | clawclawclaw-native JSON | Full (with tools + memory) | **Recommended** for new integrations |
...
The clean, clawclawclaw-native endpoint.
...
| OpenClaw (JSON) | clawclawclaw (TOML) |
|-----------------|-----------------|
...
OpenClaw stores state in `~/.openclaw/`. clawclawclaw uses configurable backends:
...
| OpenClaw Channel | clawclawclaw Status |
|------------------|-----------------|
...
For unsupported channels, point your existing integration at clawclawclaw's `/api/chat` endpoint instead of OpenClaw's `/v1/chat/completions`.
...
```typescript
// Same code as before — just point to clawclawclaw host with gateway token.
...
clawclawclaw's `process_message` starts fresh on each call. It uses **semantic memory recall** (SQLite hybrid search with embeddings + BM25) to surface relevant past context, not ordered conversation history.
...
For full conversation history support, a follow-up change to clawclawclaw's agent loop would be needed to accept a `messages[]` parameter directly.
...
# Specify output path
python scripts/convert-openclaw-config.py ~/.openclaw/openclaw.json -o ~/.clawclawclaw/config.toml
...
- [ ] Set API key: `export CLAWCLAWCLAW_API_KEY='...'`
...
**405 on /v1/chat/completions:** The endpoint isn't registered. Make sure you're running a clawclawclaw build that includes the `openclaw_compat` module (check `src/gateway/mod.rs` for the route registration).
