# Network Deployment — clawclawclaw on Raspberry Pi and Local Network

This document covers deploying clawclawclaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | clawclawclaw polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | clawclawclaw syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — clawclawclaw connects to external servers/relays. No port forwarding or public IP required.

---

## 2. clawclawclaw on Raspberry Pi
...
- Telegram channel works: clawclawclaw polls Telegram API (outbound)
...
```bash
clawclawclaw daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `clawclawclaw tunnel` (see tunnel docs).

3. clawclawclaw will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- clawclawclaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `clawclawclaw daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `clawclawclaw daemon` service).
...
- [ ] Run `clawclawclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
clawclawclaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `clawclawclaw` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo clawclawclaw service install
```

This creates:
- Init script: `/etc/init.d/clawclawclaw`
- Config directory: `/etc/clawclawclaw/`
- Log directory: `/var/log/clawclawclaw/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo clawclawclaw service install` automatically prepares `/etc/clawclawclaw`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `clawclawclaw` service user.

If no prior runtime state is available to migrate, create `/etc/clawclawclaw/config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add clawclawclaw default

# Start the service
sudo rc-service clawclawclaw start

# Check status
sudo rc-service clawclawclaw status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service clawclawclaw start` | Start the daemon |
| `sudo rc-service clawclawclaw stop` | Stop the daemon |
| `sudo rc-service clawclawclaw status` | Check service status |
| `sudo rc-service clawclawclaw restart` | Restart the daemon |
| `sudo clawclawclaw service status` | clawclawclaw status wrapper (uses `/etc/clawclawclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/clawclawclaw/access.log` |
| Errors/stderr | `/var/log/clawclawclaw/error.log` |

View logs:

```bash
sudo tail -f /var/log/clawclawclaw/error.log
```
...
sudo rc-service clawclawclaw stop
sudo rc-update del clawclawclaw default

# Remove init script
sudo clawclawclaw service uninstall
```
...
- The service runs as the `clawclawclaw:clawclawclaw` user (least privilege)
- Config must be at `/etc/clawclawclaw/config.toml` (explicit path in init script)
- If the `clawclawclaw` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo clawclawclaw service install`
- [ ] Enable: `sudo rc-update add clawclawclaw default`
- [ ] Start: `sudo rc-service clawclawclaw start`
- [ ] Verify: `sudo rc-service clawclawclaw status`
- [ ] Check logs: `/var/log/clawclawclaw/error.log`
