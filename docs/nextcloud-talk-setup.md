...
This guide covers native Nextcloud Talk integration for clawclawclaw.
...
Add this section in `~/.clawclawclaw/config.toml`:
...
clawclawclaw daemon
# or
clawclawclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, clawclawclaw verifies:
...
- clawclawclaw ignores bot-originated webhook events (`actorType = bots`).
- clawclawclaw ignores non-message/system events.
...
3. Confirm clawclawclaw receives and replies in the same room.
