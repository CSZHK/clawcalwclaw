# clawclawclaw on Nucleo-F401RE — Step-by-Step Guide

Run clawclawclaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

clawclawclaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
clawclawclaw includes everything for Nucleo-F401RE:
...
| Flash command | `clawclawclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via clawclawclaw

From the clawclawclaw repo root:

```bash
clawclawclaw peripheral flash-nucleo
```

This builds `firmware/clawclawclaw-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
Add to `~/.clawclawclaw/config.toml`:
...
| 5 | `clawclawclaw daemon` or `clawclawclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `clawclawclaw peripheral list` to verify.
