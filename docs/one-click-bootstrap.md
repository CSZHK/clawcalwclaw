...
This page defines the fastest supported path to install and initialize clawclawclaw.
...
Default behavior is **app-only** (build/install clawclawclaw) and expects existing Rust toolchain.
...
This builds a local clawclawclaw image and launches onboarding inside a container while
persisting config/workspace to `./.clawclawclaw-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
bootstrap auto-falls back to `podman`. You can also set `CLAWCLAWCLAW_CONTAINER_CLI`
explicitly (for example: `CLAWCLAWCLAW_CONTAINER_CLI=podman ./bootstrap.sh --docker`).
...
Docker tag (`CLAWCLAWCLAW_DOCKER_IMAGE`, default: `clawclawclaw-bootstrap:local`); if missing,
it pulls `ghcr.io/CSZHK/clawcalwclaw:latest` and tags it locally before running.
...
```bash
CLAWCLAWCLAW_API_KEY="sk-..." CLAWCLAWCLAW_PROVIDER="openrouter" ./bootstrap.sh --onboard
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/CSZHK/clawcalwclaw:latest`)
- `--skip-install`
- `--provider <id>`
