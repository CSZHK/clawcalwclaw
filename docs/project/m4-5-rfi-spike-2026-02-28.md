...
Issue: [#2263](https://github.com/CSZHK/clawcalwclaw/issues/2263)
...
```bash
scripts/ci/m4_5_rfi_baseline.sh /tmp/clawclawclaw-m4rfi-target
```
...
1. `crates/clawclawclaw-types`
- shared DTOs, enums, IDs, lightweight cross-domain traits
- no provider/channel/network dependencies

1. `crates/clawclawclaw-core`
- config structs + validation, provider trait contracts, routing primitives, policy helpers
- depends on `clawclawclaw-types`

1. `crates/clawclawclaw-memory`
- memory traits/backends + hygiene/snapshot plumbing
- depends on `clawclawclaw-types`, `clawclawclaw-core` contracts only where required

1. `crates/clawclawclaw-channels`
- channel adapters + inbound normalization
- depends on `clawclawclaw-types`, `clawclawclaw-core`, `clawclawclaw-memory`

1. `crates/clawclawclaw-api`
- gateway/webhook/http orchestration
- depends on `clawclawclaw-types`, `clawclawclaw-core`, `clawclawclaw-memory`, `clawclawclaw-channels`

1. `crates/clawclawclaw-bin` (or keep root binary package name stable)
...
- keep provider-specific SDK deps out of `clawclawclaw-types`
...
- add package-focused lanes for extracted crates (`-p clawclawclaw-types`, `-p clawclawclaw-core`, etc.)
...
- extract low-churn shared types into `clawclawclaw-types`
- add re-export shim layer to preserve existing import paths

### PR-3 (S)

- extract config/provider contracts into `clawclawclaw-core`
