# clawclawclaw Providers Reference
...
3. Generic fallback env vars: `CLAWCLAWCLAW_API_KEY` then `API_KEY`
...
- **Quota Tracking**: `clawclawclaw providers-quota --provider qwen-code` shows static quota info (`?/1000` - unknown remaining, 1000/day total)
...
clawclawclaw onboard --provider volcengine --api-key "$ARK_API_KEY" --model doubao-1-5-pro-32k-250115 --force
```

Quick validation:

```bash
clawclawclaw models refresh --provider volcengine
clawclawclaw agent --provider volcengine --model doubao-1-5-pro-32k-250115 -m "ping"
```
...
clawclawclaw onboard --provider stepfun --api-key "$STEP_API_KEY" --model step-3.5-flash --force
```

Quick validation:

```bash
clawclawclaw models refresh --provider stepfun
clawclawclaw agent --provider stepfun --model step-3.5-flash -m "ping"
```
...
clawclawclaw onboard --provider siliconflow --api-key "$SILICONFLOW_API_KEY" --model Pro/zai-org/GLM-4.7 --force
```

Quick validation:

```bash
clawclawclaw models refresh --provider siliconflow
clawclawclaw agent --provider siliconflow --model Pro/zai-org/GLM-4.7 -m "ping"
```
...
- After multimodal normalization, clawclawclaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, clawclawclaw returns a structured capability error instead of silently ignoring images.
...
- clawclawclaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `clawclawclaw models refresh --provider llamacpp`
...
- Model discovery: `clawclawclaw models refresh --provider sglang`
...
- Model discovery: `clawclawclaw models refresh --provider vllm`
...
- Model discovery: `clawclawclaw models refresh --provider osaurus`
...
Since clawclawclaw cannot auto-detect this, you can override it in `config.toml`:
...
Environment override: `CLAWCLAWCLAW_MODEL_SUPPORT_VISION=true`
...
- When set, overrides `CLAWCLAWCLAW_CODEX_REASONING_EFFORT`.
...
- Model discovery: `clawclawclaw models refresh --provider nvidia`
