...
(skills) in clawclawclaw. WASM tools let you extend the agent with custom capabilities
written in any language that compiles to WebAssembly — without modifying clawclawclaw's
core source code.
...
7. [How clawclawclaw Loads and Uses the Tool](#7-how-clawclawclaw-loads-and-uses-the-tool)
...
└───────────────────────┬─────────────────────────────────────┘
                        │  WASI stdio protocol
┌───────────────────────▼─────────────────────────────────────┐
│  clawclawclaw WASM engine (wasmtime + WASI)                     │
│                                                             │
...
| Requirement | Purpose |
|---|---|
| clawclawclaw built with `--features wasm-tools` | Enables the WASM runtime |
| `wasmtime` CLI | Local testing (`clawclawclaw skill test`) |
...
```bash
clawclawclaw skill new <name> --template <typescript|rust|go|python>
```

Example:

```bash
clawclawclaw skill new weather_lookup --template rust
```
...
**Input** (written to the tool's stdin by clawclawclaw):
...
`parameters`. clawclawclaw passes the LLM-provided argument object verbatim.

**Output** (read from the tool's stdout by clawclawclaw):
...
clawclawclaw the tool's name, description, and the JSON Schema for its parameters.
...
| `homepage` | no | Optional URL shown in `clawclawclaw skill list` |
...
componentize-py -d wit/ -w clawclawclaw-skill componentize app -o tool.wasm
```
...
| Python | `componentize-py -d wit/ -w clawclawclaw-skill componentize app -o tool.wasm` | `tool.wasm` |
...
Before installing, test the tool directly without starting the full clawclawclaw agent:

```bash
clawclawclaw skill test . --args '{"city":"Hanoi","units":"metric"}'
```

You can also test an installed skill by name:

```bash
clawclawclaw skill test weather_lookup --args '{"city":"Tokyo"}'
```

Or test a specific tool inside a multi-tool skill:

```bash
clawclawclaw skill test . --tool my_tool_name --args '{"city":"Paris"}'
```
...
```bash
clawclawclaw skill install ./weather_lookup
```

This copies your skill directory into `<workspace>/skills/weather_lookup/`.
clawclawclaw will auto-discover it on next startup.

### 6.2 Install from a git repository

```bash
clawclawclaw skill install https://github.com/yourname/weather_lookup.git
```

clawclawclaw clones the repository into the skills directory and scans for WASM tools.

### 6.3 Install from ZeroMarket registry

```bash
# Format: namespace/package-name
clawclawclaw skill install acme/weather-lookup

# With a specific version
clawclawclaw skill install acme/weather-lookup@0.2.1
```

clawclawclaw fetches the package index from the configured registry URL, then downloads
`tool.wasm` and `manifest.json` for each tool in the package.

**Verify the install:**

```bash
clawclawclaw skill list
```

---

## 7. How clawclawclaw Loads and Uses the Tool

### 7.1 Startup discovery

Every time the clawclawclaw agent starts, it scans the `skills/` directory and loads
...
Agent: [internally, LLM selects tool "weather_lookup" with args {"city":"Hanoi"}]
       ...
       clawclawclaw calls weather_lookup WASM tool:
         stdin  → {"city":"Hanoi"}
...
If a tool fails (non-zero exit, invalid JSON, timeout, fuel exhaustion), clawclawclaw
...
**Installed layout** (created by `clawclawclaw skill install`):
...
Add this section to your `clawclawclaw.toml` to tune WASM tool behavior:
...
# Registry URL used by `clawclawclaw skill install namespace/package`
...
- Crash the clawclawclaw process
...
**Tool not appearing in `clawclawclaw skill list`**
