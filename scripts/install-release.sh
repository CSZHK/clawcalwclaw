...
pick_install_dir() {
  if [ -n "${CLAWCLAWCLAW_INSTALL_DIR:-}" ]; then
    echo "$CLAWCLAWCLAW_INSTALL_DIR"
    return 0
  fi
...
Installs the latest Linux clawclawclaw binary from official GitHub releases.

Options:
  --no-onboard   Install only; do not run `clawclawclaw onboard`

Environment:
  CLAWCLAWCLAW_INSTALL_DIR  Override install directory
EOF
      exit 0
      ;;
    *)
...
TRIPLE="$(linux_triple)"
ASSET="clawclawclaw-${TRIPLE}.tar.gz"
...
if [ ! -f "$TMP_DIR/clawclawclaw" ]; then
  echo "error: release archive does not contain expected 'clawclawclaw' binary" >&2
  exit 1
fi

INSTALL_DIR="$(pick_install_dir)"
BIN_PATH="${INSTALL_DIR}/clawclawclaw"

if [ "${INSTALL_DIR#/usr/local/}" != "$INSTALL_DIR" ]; then
  run_privileged mkdir -p "$INSTALL_DIR"
  run_privileged install -m 0755 "$TMP_DIR/clawclawclaw" "$BIN_PATH"
else
  mkdir -p "$INSTALL_DIR"
  install -m 0755 "$TMP_DIR/clawclawclaw" "$BIN_PATH"
fi

echo "==> Installed: $BIN_PATH"
if ! command -v clawclawclaw >/dev/null 2>&1; then
