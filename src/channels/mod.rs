...
//! clawclawclaw to external platforms. Each channel implements the [`Channel`] trait
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "clawclawclaw.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "clawclawclaw.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["clawclawclaw", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["clawclawclaw", "restart"];
    ...
    match channel_name {
        ...
        "whatsapp" => Some(
            "When responding on WhatsApp:\n\
             ...
             - The path MUST be an absolute filesystem path to a local file (e.g. [IMAGE:/home/nicolas/.clawclawclaw/workspace/images/chart.png]).\n\
             - Keep normal text outside markers and never wrap markers in code fences.\n\
...
async fn load_runtime_defaults_from_config_file(
    ...
    if let Some(clawclawclaw_dir) = path.parent() {
        let store = crate::security::SecretStore::new(clawclawclaw_dir, parsed.secrets.encrypt);
...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `clawclawclaw models refresh --provider {}`.",
...
pub fn build_system_prompt_with_mode(
    ...
    if prompt.is_empty() {
        "You are clawclawclaw, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct."
            .to_string()
    } else {
        prompt
    }
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `clawclawclaw onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `clawclawclaw daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `clawclawclaw service stop && clawclawclaw service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.clawclawclaw.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.clawclawclaw.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.clawclawclaw.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.clawclawclaw.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/clawclawclaw");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: clawclawclaw channel start");
            println!("To check health:    clawclawclaw channel doctor");
            println!("To configure:      clawclawclaw onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `clawclawclaw onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.clawclawclaw/config.toml directly");
        }
...
fn collect_configured_channels(
    ...
    if let Some(ref mx) = config.channels_config.matrix {
        channels.push(ConfiguredChannel {
            display_name: "Matrix",
            channel: Arc::new(
                MatrixChannel::new_with_session_hint_and_clawclawclaw_dir(
                    mx.homeserver.clone(),
    ...
    if config.channels_config.lark.is_some() || config.channels_config.feishu.is_some() {
        ...
        tracing::warn!(
            "Lark/Feishu channel is configured but this binary was compiled without `channel-lark`; skipping Lark/Feishu startup. \
             binary={executable}. \
             If you built from source, run the built artifact directly (for example `./target/release/clawclawclaw daemon`) \
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() && init_failures.is_empty() {
        println!("No real-time channels configured. Run `clawclawclaw onboard` first.");
        return Ok(());
    }

    println!("🩺 clawclawclaw Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `clawclawclaw gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    if configured_channels.is_empty() && init_failures.is_empty() {
        println!("No channels configured. Run `clawclawclaw onboard` to set up channels.");
        return Ok(());
    }
    ...
    println!("🦀 clawclawclaw Channel Server");
    println!("  🤖 Model:    {model}");
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_handles_approve_command_without_llm_call() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(crate::config::ReliabilityConfig::default()),
            provider_runtime_options: providers::ProviderRuntimeOptions {
                clawclawclaw_dir: Some(temp.path().to_path_buf()),
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(
            prompt.contains("Name: clawclawclaw"),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from clawclawclaw 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
BTC is currently around $65,000 based on latest tool output."#;
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "clawclawclaw.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "clawclawclaw.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["clawclawclaw", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["clawclawclaw", "restart"]);
    }
    ...
    async fn e2e_photo_attachment_rejected_by_non_vision_provider() {
        ...
        process_channel_message(
            ...
            traits::ChannelMessage {
                ...
                sender: "clawclawclaw_user".to_string(),
    ...
    async fn e2e_failed_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .get("test-channel_clawclawclaw_user")
