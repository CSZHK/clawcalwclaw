...
//!
//! Wire protocol (clawclawclaw serial JSON):
//! ```text
...
/// Default baud rate for clawclawclaw serial devices.
...
/// Serial transport for clawclawclaw hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify clawclawclaw firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "clawclawclaw"`.
    ///
    /// Returns `true` if a clawclawclaw device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "clawclawclaw" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
                        .and_then(|v| v.as_str())
                        .map(|s| s == "clawclawclaw")
                        .unwrap_or(false)
            }
            _ => false,
        }
