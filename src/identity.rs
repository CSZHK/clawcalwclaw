...
//! JSON to clawclawclaw's system prompt format.
...
const IDENTITY_BACKENDS: [IdentityBackendProfile; 2] = [
    IdentityBackendProfile {
        ...
        description: "Classic clawclawclaw layout with IDENTITY.md, SOUL.md, USER.md, and friends.",
    },
    IdentityBackendProfile {
        key: "aieos",
...
pub fn generate_default_aieos_json(agent_name: &str, user_name: &str) -> String {
    ...
    serde_json::json!({
        "identity": {
            ...
            "bio": format!(
                "{resolved_agent_name} is a clawclawclaw assistant focused on helping {resolved_user_name} get work done efficiently."
            ),
            "origin": "clawclawclaw",
            "residence": "Workspace"
        },
...
/// with clawclawclaw's agent system.
...
mod tests {
    ...
    fn test_workspace_dir() -> PathBuf {
        std::env::temp_dir().join("clawclawclaw-test-identity")
    }
