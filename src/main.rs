...
const CLAWCLAWCLAW_BUILD_VERSION: &str = env!("CLAWCLAWCLAW_BUILD_VERSION");
...
/// `clawclawclaw` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "clawclawclaw")]
#[command(author = "theonlyhennygod")]
#[command(version = CLAWCLAWCLAW_BUILD_VERSION)]
...
Examples:
  clawclawclaw agent                              # interactive session
  clawclawclaw agent -m \"Summarize today's logs\"  # single message
  clawclawclaw agent -p anthropic --model claude-sonnet-4-20250514
  clawclawclaw agent --peripheral nucleo-f401re:/dev/ttyACM0
  clawclawclaw agent --autonomy-level full --max-actions-per-hour 100
  clawclawclaw agent -m \"quick task\" --memory-backend none --compact-context")]
    Agent {
...
Examples:
  clawclawclaw tui
  clawclawclaw tui --provider anthropic --model claude-sonnet-4-20250514")]
    Tui {
        /// Provider to use (openrouter, anthropic, openai, openai-codex)
        #[arg(short, long)]
        provider: Option<String>,
...
Examples:
  clawclawclaw gateway                  # use config defaults
  clawclawclaw gateway -p 8080          # listen on port 8080
  clawclawclaw gateway --host 0.0.0.0   # bind to all interfaces
  clawclawclaw gateway -p 0             # random available port
  clawclawclaw gateway --new-pairing    # clear tokens and generate fresh pairing code")]
    Gateway {
...
Launches the full clawclawclaw runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run clawclawclaw in \
production or as an always-on assistant.

Use 'clawclawclaw service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  clawclawclaw daemon                   # use config defaults
  clawclawclaw daemon -p 9090           # gateway on port 9090
  clawclawclaw daemon --host 127.0.0.1  # localhost only")]
    Daemon {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
    ...
    /// Self-update clawclawclaw to the latest version
    #[command(long_about = "\
Self-update clawclawclaw to the latest release from GitHub.
...
Examples:
  clawclawclaw update              # Update to latest version
  clawclawclaw update --check      # Check for updates without installing
  clawclawclaw update --instructions # Show install-method-specific update instructions
  clawclawclaw update --force      # Reinstall even if already up to date")]
    Update {
    ...
    /// - `clawclawclaw estop --level domain-block --domain "*.chase.com"`
    ...
    Estop {
        ...
        /// Level used when engaging estop from `clawclawclaw estop`.
        #[arg(long, value_enum)]
...
Examples:
  ...
  clawclawclaw cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York
  clawclawclaw cron add '*/30 * * * *' 'Check system health'
  clawclawclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  clawclawclaw cron add-every 60000 'Ping heartbeat'
  clawclawclaw cron once 30m 'Run backup in 30 minutes'
  clawclawclaw cron pause <task-id>
  clawclawclaw cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Examples:
  clawclawclaw providers-quota                    # text output, all providers
  clawclawclaw providers-quota --format json      # JSON output
  clawclawclaw providers-quota --provider gemini  # filter by provider"
    )]
    ProvidersQuota {
...
Add, remove, list, and health-check channels that connect clawclawclaw \
...
Examples:
  ...
  clawclawclaw channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  clawclawclaw channel remove my-bot
  clawclawclaw channel bind-telegram clawclawclaw_user")]
    Channel {
        #[command(subcommand)]
...
Examples:
  ...
  clawclawclaw hardware introspect /dev/ttyACM0
  clawclawclaw hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: clawclawclaw::HardwareCommands,
    },
...
Examples:
  ...
  clawclawclaw peripheral add rpi-gpio native
  clawclawclaw peripheral flash --port /dev/cu.usbmodem12345
  clawclawclaw peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: clawclawclaw::PeripheralCommands,
    },
...
Examples:
  clawclawclaw memory stats
  clawclawclaw memory list
  clawclawclaw memory list --category core --limit 10
  clawclawclaw memory get <key>
  clawclawclaw memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Manage clawclawclaw configuration.

Inspect, query, and modify configuration settings.

Examples:
  clawclawclaw config show                        # show effective config (secrets masked)
  clawclawclaw config get gateway.port            # query a specific value by dot-path
  clawclawclaw config set gateway.port 8080       # update a value and save to config.toml
  clawclawclaw config schema                      # print full JSON Schema to stdout")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Generate shell completion scripts for `clawclawclaw`.

The script is printed to stdout so it can be sourced directly:

Examples:
  source <(clawclawclaw completions bash)
  clawclawclaw completions zsh > ~/.zfunc/_clawclawclaw
  clawclawclaw completions fish > ~/.config/fish/completions/clawclawclaw.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        std::env::set_var("CLAWCLAWCLAW_CONFIG_DIR", config_dir);
    }
    ...
    } = &cli.command
    {
        ...
        if std::env::var("CLAWCLAWCLAW_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for clawclawclaw.");
    ...
    match cli.command {
        ...
        Commands::Gateway {
            ...
            if port == 0 {
                info!("🚀 Starting clawclawclaw Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting clawclawclaw Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host } => {
            ...
            let host = host.unwrap_or_else(|| config.gateway.host.clone());
            if port == 0 {
                info!("🧠 Starting clawclawclaw Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting clawclawclaw Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 clawclawclaw Status");
            println!();
            println!("Version:     {}", CLAWCLAWCLAW_BUILD_VERSION);
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `clawclawclaw auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `clawclawclaw auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                    ...
                    let result = async {
                        let pending =
                            load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                                ...
                                clawclawclaw auth login --provider openai-codex --profile {}\n\n\
                    ...
                    let result = async {
                        let pending =
                            load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                                ...
                                💡 Please start the login flow first:\n   \
                                clawclawclaw auth login --provider gemini --profile {}\n\n\
                                Then paste the callback URL or code here.",
                                    profile
                                )
                            })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `clawclawclaw auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `clawclawclaw auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["clawclawclaw", "completions", shell])
    ...
    fn gateway_cli_accepts_new_pairing_flag() {
        let cli = Cli::try_parse_from(["clawclawclaw", "gateway", "--new-pairing"])
    ...
    fn gateway_cli_defaults_new_pairing_to_false() {
        let cli = Cli::try_parse_from(["clawclawclaw", "gateway"]).expect("gateway should parse");
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["clawclawclaw", "onboard", "--force"])
    ...
    fn onboard_cli_accepts_no_totp_flag() {
        let cli = Cli::try_parse_from(["clawclawclaw", "onboard", "--no-totp"])
    ...
    fn config_help_mentions_show_get_set_examples() {
        ...
        assert!(help.contains("clawclawclaw config show"));
        assert!(help.contains("clawclawclaw config get gateway.port"));
        assert!(help.contains("clawclawclaw config set gateway.port 8080"));
    }
    ...
    fn config_cli_parses_show_get_set_subcommands() {
        ...
        let get = Cli::try_parse_from(["clawclawclaw", "config", "get", "gateway.port"])
        ...
        let set = Cli::try_parse_from(["clawclawclaw", "config", "set", "gateway.port", "8080"])
    ...
    fn update_cli_parses_instructions_flag() {
        let cli = Cli::try_parse_from(["clawclawclaw", "update", "--instructions"])
