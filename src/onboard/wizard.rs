...
pub async fn run_wizard_with_migration(
    ...
    println!(
        "  {}",
        style("Welcome to clawclawclaw — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 11, "Channels (How You Talk to clawclawclaw)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("CLAWCLAWCLAW_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to clawclawclaw)");
...
/// Use `clawclawclaw onboard` or `clawclawclaw onboard --api-key sk-... --provider openrouter --memory sqlite|lucid|cortex-mem`.
/// Use `clawclawclaw onboard --interactive` for the full wizard.
...
async fn maybe_run_openclaw_migration(
    ...
    let should_run = if options.enabled {
        ...
        Confirm::new()
            .with_prompt(
                "  Merge OpenClaw data into this clawclawclaw workspace now? (preserve existing data)",
            )
            .default(true)
            .interact()?
    } else {
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    let config_dir = home.join(".clawclawclaw");
...
async fn run_quick_setup_with_home(
    ...
    let (clawclawclaw_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     clawclawclaw agent -m \"Hello!\"");
            println!("    2. Gateway:  clawclawclaw gateway");
            println!("    3. Status:   clawclawclaw status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             clawclawclaw auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              clawclawclaw agent -m \"Hello!\"");
                println!("    3. Gateway:           clawclawclaw gateway");
                println!("    4. Status:            clawclawclaw status");
            }
        } else {
            ...
            println!("    2. Or edit:           ~/.clawclawclaw/config.toml");
            println!("    3. Chat:              clawclawclaw agent -m \"Hello!\"");
            println!("    4. Gateway:           clawclawclaw gateway");
        }
    } else {
        println!("    1. Chat:     clawclawclaw agent -m \"Hello!\"");
        println!("    2. Gateway:  clawclawclaw gateway");
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `clawclawclaw models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: clawclawclaw models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if key.trim().is_empty() {
            print_bullet(
                "No token provided. clawclawclaw will open the GitHub device login flow on first use.",
            );
        }
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("clawclawclaw will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how clawclawclaw connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("clawclawclaw uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("clawclawclaw can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("clawclawclaw can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should clawclawclaw interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how clawclawclaw stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_identity_backend() -> Result<IdentityConfig> {
    print_bullet("Choose the identity format clawclawclaw should scaffold for this workspace.");
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to clawclawclaw from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to clawclawclaw from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to clawclawclaw from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to clawclawclaw from Slack").dim()
                );
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.clawclawclaw/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("clawclawclaw-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via CLAWCLAWCLAW_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::LarkFeishu => {
                ...
                println!(
                    ...
                    style("— talk to clawclawclaw from Lark or Feishu").dim()
                );
                print_bullet(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("clawclawclaw will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("clawclawclaw is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if canonical_provider == "copilot" {
            ...
            println!(
                "       {}",
                style("clawclawclaw auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: clawclawclaw auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("clawclawclaw channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("clawclawclaw agent -m \"Hello, clawclawclaw!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("clawclawclaw agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("clawclawclaw status").yellow());
...
mod tests {
    ...
    async fn run_quick_setup_with_clean_env(
        ...
        let _workspace_env = EnvVarGuard::unset("CLAWCLAWCLAW_WORKSPACE");
        let _config_env = EnvVarGuard::unset("CLAWCLAWCLAW_CONFIG_DIR");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        let tmp = TempDir::new().unwrap();
        let clawclawclaw_dir = tmp.path().join(".clawclawclaw");
        let config_path = clawclawclaw_dir.join("config.toml");

        tokio::fs::create_dir_all(&clawclawclaw_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("clawclawclaw-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".clawclawclaw").join("config.toml");
        ...
        .await
        .expect("quick setup should honor CLAWCLAWCLAW_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** clawclawclaw"),
            "should default agent name to clawclawclaw"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let ctx = ProjectContext {
            ...
            agent_name: "clawclawclaw-v2".into(),
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("clawclawclaw-v2"));
    }
    ...
    fn memory_config_defaults_for_hybrid_enable_sqlite_hygiene() {
        ...
        assert_eq!(config.purge_after_days, 30);
        assert_eq!(config.embedding_cache_size, 10000);
        assert_eq!(config.qdrant.collection, "clawclawclaw_memories");
    }
