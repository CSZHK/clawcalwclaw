//! Flash clawclawclaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash clawclawclaw Nucleo firmware. Builds from firmware/clawclawclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (clawclawclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("clawclawclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from clawclawclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building clawclawclaw Nucleo firmware...");
    ...
    println!("clawclawclaw Nucleo firmware flashed successfully.");
