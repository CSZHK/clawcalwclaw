...
//! extension directories for subdirectories containing `clawclawclaw.plugin.toml`.
...
pub fn discover_plugins(workspace_dir: Option<&Path>, extra_paths: &[PathBuf]) -> DiscoveryResult {
    ...
    if let Some(home) = dirs_home() {
        let global = home.join(".clawclawclaw").join("extensions");
    ...
    if let Some(ws) = workspace_dir {
        let ws_ext = ws.join(".clawclawclaw").join("extensions");
...
"#
    ...
    fn discover_from_workspace() {
        ...
        let ext_dir = ws.join(".clawclawclaw").join("extensions");
