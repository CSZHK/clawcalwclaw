//! Plugin manifest — the `clawclawclaw.plugin.toml` descriptor.
//!
//! Mirrors OpenClaw's `openclaw.plugin.json` but uses TOML to match
//! clawclawclaw's existing config format.
...
pub const PLUGIN_MANIFEST_FILENAME: &str = "clawclawclaw.plugin.toml";
...
/// Load and parse `clawclawclaw.plugin.toml` from `root_dir`.
...
fn required_wit_package_for_capability(capability: &PluginCapability) -> &'static str {
    match capability {
        PluginCapability::Hooks | PluginCapability::ModifyToolResults => "clawclawclaw:hooks",
        PluginCapability::Tools => "clawclawclaw:tools",
        PluginCapability::Providers => "clawclawclaw:providers",
    }
}
...
pub fn validate_manifest_with_profile(
    ...
    if !manifest.providers.is_empty() && !declared_wit_packages.contains("clawclawclaw:providers") {
        anyhow::bail!(
            "plugin providers require wit package 'clawclawclaw:providers@{SUPPORTED_WIT_MAJOR}.x'"
        );
    }
...
"#,
    ...
    fn manifest_requires_id_and_module_path_for_runtime_validation() {
        ...
        let valid = PluginManifest {
            ...
            wit_packages: vec!["clawclawclaw:hooks@1.0.0".into()],
            tools: vec![],
            providers: vec![],
        };
    ...
    fn manifest_rejects_unknown_wit_package() {
        let manifest = PluginManifest {
            ...
            capabilities: vec![],
            module_path: "plugins/demo.wasm".into(),
            wit_packages: vec!["clawclawclaw:unknown@1.0.0".into()],
            tools: vec![],
            providers: vec![],
        };
