...
//! The default clawclawclaw binary excludes it to maintain the 4.6 MB size target.
...
mod tests {
    ...
    fn wasm_storage_path_default() {
        let rt = WasmRuntime::new(default_config());
        assert!(rt.storage_path().to_string_lossy().contains("clawclawclaw"));
    }
    ...
    fn wasm_storage_path_with_workspace() {
        ...
        assert_eq!(
            rt.storage_path(),
            PathBuf::from("/home/user/project/.clawclawclaw")
        );
    }
