...
const OTP_ISSUER: &str = "clawclawclaw";
...
impl OtpValidator {
    pub fn from_config(
        config: &OtpConfig,
        clawclawclaw_dir: &Path,
        store: &SecretStore,
    ) -> Result<(Self, Option<String>)> {
        let secret_path = secret_file_path(clawclawclaw_dir);
...
pub fn secret_file_path(clawclawclaw_dir: &Path) -> PathBuf {
    clawclawclaw_dir.join(OTP_SECRET_FILE)
}
