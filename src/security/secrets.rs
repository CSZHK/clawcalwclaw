...
// in `~/.clawclawclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.clawclawclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(clawclawclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: clawclawclaw_dir.join(".secret_key"),
            enabled,
        }
    }
