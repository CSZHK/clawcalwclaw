...
const SERVICE_LABEL: &str = "com.clawclawclaw.daemon";
const WINDOWS_TASK_NAME: &str = "clawclawclaw Daemon";
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["clawclawclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/clawclawclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/clawclawclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "clawclawclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove clawclawclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/clawclawclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    let unit = format!(
        "[Unit]\nDescription=clawclawclaw daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={exe} daemon\nRestart=always\nRestartSec=3\n{env_lines}\n[Install]\nWantedBy=default.target\n",
        exe = exe.display(),
        env_lines = env_lines,
    );
...
/// Check if the clawclawclaw user exists and has expected properties.
...
fn check_clawclawclaw_user() -> Result<()> {
    ...
    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser clawclawclaw && delgroup clawclawclaw",
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'clawclawclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'clawclawclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/clawclawclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: clawclawclaw user has home directory '{}' (expected /var/lib/clawclawclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_clawclawclaw_user() -> Result<()> {
    ...
    if is_alpine {
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "clawclawclaw"])
                .output()
                .context("Failed to create clawclawclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create clawclawclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: clawclawclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create clawclawclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create clawclawclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "clawclawclaw"])
            .output()
            .context("Failed to create clawclawclaw user")?;
    ...
    println!("✅ Created system user: clawclawclaw");
    Ok(())
}

/// Change ownership of a path to clawclawclaw:clawclawclaw
#[cfg(unix)]
fn chown_to_clawclawclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["clawclawclaw:clawclawclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to clawclawclaw:clawclawclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_clawclawclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_clawclawclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "clawclawclaw:clawclawclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to clawclawclaw:clawclawclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_clawclawclaw(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn resolve_invoking_user_config_dir() -> Option<PathBuf> {
    let resolve_home_config_dir = |home: PathBuf| {
        let preferred = home.join(".clawclawclaw");
        if preferred.exists() {
            return preferred;
        }
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(resolve_home_config_dir(PathBuf::from(fields[5])));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'clawclawclaw' cannot write {} ({details}). \
             Re-run `sudo clawclawclaw service install` and ensure ownership is clawclawclaw:clawclawclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/clawclawclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="clawclawclaw:clawclawclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/clawclawclaw/access.log"
error_log="/var/log/clawclawclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/clawclawclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo clawclawclaw service install"
        );
    }

    ensure_clawclawclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/clawclawclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/clawclawclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_clawclawclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by clawclawclaw:clawclawclaw)",
    ...
    chown_recursive_to_clawclawclaw(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/clawclawclaw");
    ...
    run_checked(Command::new("rc-update").args(["add", "clawclawclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/clawclawclaw");
    println!("   Config path: /etc/clawclawclaw/config.toml");
    println!("   Start with: sudo clawclawclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("clawclawclaw-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/clawclawclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "clawclawclaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/clawclawclaw");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/clawclawclaw"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"clawclawclaw\""));
        assert!(script.contains("description=\"clawclawclaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/clawclawclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/clawclawclaw daemon\""));
        assert!(!script.contains("env CLAWCLAWCLAW_CONFIG_DIR"));
        assert!(!script.contains("env CLAWCLAWCLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"clawclawclaw:clawclawclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/clawclawclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/clawclawclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/clawclawclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/clawclawclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/clawclawclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/clawclawclaw"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/clawclawclaw'".to_string()
            ]
        );
    }
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/clawclawclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/clawclawclaw/workspace'".to_string(),
                "clawclawclaw".to_string()
            ]
        );
    }
