//! Integrator — generates clawclawclaw-standard SKILL.toml + SKILL.md from scout results.
...
```toml
# Add to your clawclawclaw config:
[skills.{name}]
enabled = true
```
...
mod tests {
    ...
    async fn integrate_creates_files() {
        let tmp = std::env::temp_dir().join("clawclawclaw-test-integrate");
