...
const OPEN_SKILLS_SYNC_MARKER: &str = ".clawclawclaw-open-skills-sync";
const OPEN_SKILLS_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24 * 7;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.clawclawclaw/workspace/skills/<name>/SKILL.md`
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("CLAWCLAWCLAW_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("CLAWCLAWCLAW_OPEN_SKILLS_DIR").ok();
    let home_dir = UserDirs::new().map(|dirs| dirs.home_dir().to_path_buf());
...
// ─── Scaffold (clawclawclaw skill new) ───────────────────────────────────────────
...
pub fn scaffold_skill(
    ...
    let tmpl = templates::find(template_name).ok_or_else(|| {
        ...
        anyhow::anyhow!(
            "Unknown template '{template_name}'. Run 'clawclawclaw skill templates' to list available templates.\nAvailable: {}",
            names.join(", ")
        )
    })?;
    ...
    std::fs::write(
        ...
        format!(
             ...
             **Example:**\n\
             ...
             ## Test\n\n\
             ```bash\n\
             clawclawclaw skill test . --args '{test_args}'\n\
             ```\n"
        ),
    )?;
    Ok(())
...
fn write_readme(dir: &std::path::Path, name: &str, language: &str, test_args: &str) -> Result<()> {
    let (build_cmd, test_note) = match language {
        ...
        "python" => (
            "componentize-py -d wit/ -w clawclawclaw-skill componentize main -o tool.wasm",
...
// ─── Local test (clawclawclaw skill test) ────────────────────────────────────────
...
///
/// Usage: `clawclawclaw skill install /path/to/skill.zip`
...
fn extract_zip_bytes_to_skills(
    ...
    if skill_dir.exists() {
        anyhow::bail!(
            "skill '{}' already exists at {}; run 'clawclawclaw skill remove {}' first",
            skill_name,
            skill_dir.display(),
            skill_name
        );
    ...
    // Write a minimal SKILL.toml so the skill appears in `clawclawclaw skill list`
    ...
    match command {
        crate::SkillCommands::New { name, template } => {
            ...
            match tmpl.language {
                ...
                "python" => {
                    ...
                    println!("    componentize-py -d wit/ -w clawclawclaw-skill componentize main -o tool.wasm");
                }
                _ => {}
            }
            println!("    clawclawclaw skill test . --args '{}'", tmpl.test_args);
            println!();
            println!(
                "  {} 'clawclawclaw skill test' requires the {} CLI:",
        ...
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.clawclawclaw/workspace/skills/my-skill");
                println!("              echo '# My Skill' > ~/.clawclawclaw/workspace/skills/my-skill/SKILL.md");
                println!();
                println!("  Or install: clawclawclaw skills install <source>");
            } else {
        ...
        crate::SkillCommands::Templates => {
            ...
            println!();
            println!("  Usage:");
            println!("    clawclawclaw skill new <name> --template <template-name>");
            println!();
            println!("  Example:");
            println!(
                "    clawclawclaw skill new my_weather --template {}",
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.clawclawclaw");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.clawclawclaw/skills"));
    }
    ...
    fn load_skills_with_config_reads_open_skills_dir_without_network() {
        ...
        let _enabled_guard = EnvVarGuard::unset("CLAWCLAWCLAW_OPEN_SKILLS_ENABLED");
        let _dir_guard = EnvVarGuard::unset("CLAWCLAWCLAW_OPEN_SKILLS_DIR");
    ...
    fn registry_install_dir_name_is_package_name_only() {
        ...
        for (source, expected_dir) in [
            ("clawclawclaw-org/weather-lookup", "weather-lookup"),
            ("clawclawclaw-org/calculator", "calculator"),
            ("clawclawclaw-user/my_tool", "my_tool"),
        ] {
    ...
    fn is_registry_source_accepts_valid_namespace_name() {
        assert!(is_registry_source("clawclawclaw/weather-lookup"));
    ...
    fn scaffold_skill_rejects_unknown_template() {
        let dir = tempfile::tempdir().unwrap();
        let result = scaffold_skill("clawclawclaw_test_tool", "cobol", dir.path());
        assert!(result.is_err());
        let msg = result.unwrap_err().to_string();
    ...
    fn scaffold_skill_rejects_existing_directory() {
        let dir = tempfile::tempdir().unwrap();
        fs::create_dir_all(dir.path().join("clawclawclaw_test_tool")).unwrap();
        let result = scaffold_skill("clawclawclaw_test_tool", "typescript", dir.path());
        assert!(result.is_err());
        let msg = result.unwrap_err().to_string();
    ...
    fn scaffold_skill_typescript_creates_required_files() {
        let dir = tempfile::tempdir().unwrap();
        scaffold_skill("clawclawclaw_test_ts", "typescript", dir.path()).unwrap();
        let skill_dir = dir.path().join("clawclawclaw_test_ts");
    ...
    fn scaffold_skill_rust_creates_required_files() {
        let dir = tempfile::tempdir().unwrap();
        scaffold_skill("clawclawclaw_test_rs", "rust", dir.path()).unwrap();
        let skill_dir = dir.path().join("clawclawclaw_test_rs");
    ...
    fn scaffold_skill_go_creates_required_files() {
        let dir = tempfile::tempdir().unwrap();
        scaffold_skill("clawclawclaw_test_go", "go", dir.path()).unwrap();
        let skill_dir = dir.path().join("clawclawclaw_test_go");
        assert!(
            skill_dir.join("manifest.json").exists(),
            "manifest.json missing"
        );
    ...
    fn scaffold_skill_gitignore_always_created() {
        for template in ["rust", "typescript", "go", "python"] {
            let dir = tempfile::tempdir().unwrap();
            let name = format!("clawclawclaw_test_{template}");
    ...
    fn scaffold_skill_skill_md_contains_name() {
        let dir = tempfile::tempdir().unwrap();
        scaffold_skill("clawclawclaw_md_check", "typescript", dir.path()).unwrap();
        let skill_md =
            fs::read_to_string(dir.path().join("clawclawclaw_md_check").join("SKILL.md")).unwrap();
        assert!(
            skill_md.contains("clawclawclaw_md_check"),
    ...
    fn is_clawhub_source_rejects_other_domains() {
        ...
        assert!(!is_clawhub_source("clawclawclaw/skill"));
    }
    ...
    fn is_zip_url_source_rejects_other_formats() {
        assert!(!is_zip_url_source("clawclawclaw/skill"));
