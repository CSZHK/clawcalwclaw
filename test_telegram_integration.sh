#!/bin/bash
# clawclawclaw Telegram Integration Test Suite
...
if [ -f "target/release/clawclawclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/clawclawclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/clawclawclaw 2>/dev/null || stat -c%s target/release/clawclawclaw)
...
CONFIG_PATH="$HOME/.clawclawclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'clawclawclaw onboard' first"
    fi
else
    warn "No config file found - run 'clawclawclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/clawclawclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
