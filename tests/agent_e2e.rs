...
//!
//! Ref: https://github.com/CSZHK/clawcalwclaw/issues/618 (item 6)

use anyhow::Result;
use async_trait::async_trait;
use clawclawclaw::agent::agent::Agent;
use clawclawclaw::agent::dispatcher::{NativeToolDispatcher, XmlToolDispatcher};
use clawclawclaw::agent::memory_loader::MemoryLoader;
use clawclawclaw::config::MemoryConfig;
use clawclawclaw::memory;
use clawclawclaw::memory::Memory;
use clawclawclaw::observability::{NoopObserver, Observer};
use clawclawclaw::providers::traits::ChatMessage;
use clawclawclaw::providers::{
...
async fn e2e_multi_turn_with_memory_enrichment() {
    ...
    assert!(req1_user.content.contains("project: clawclawclaw"));
...
async fn e2e_live_openai_codex_multi_turn() {
    use clawclawclaw::providers::openai_codex::OpenAiCodexProvider;
...
///
/// Requires valid credentials in `~/.clawclawclaw/`.
...
async fn e2e_live_research_phase() {
    use clawclawclaw::agent::research::{run_research_phase, should_trigger};
    use clawclawclaw::config::{ResearchPhaseConfig, ResearchTrigger};
    use clawclawclaw::observability::NoopObserver;
    use clawclawclaw::providers::openai_codex::OpenAiCodexProvider;
    use clawclawclaw::tools::{Tool, ToolResult};
    use std::sync::Arc;
    ...
    let observer: Arc<dyn clawclawclaw::observability::Observer> = Arc::new(NoopObserver);
...
async fn e2e_agent_research_prompt_guided() {
    use clawclawclaw::config::{ResearchPhaseConfig, ResearchTrigger};
    use clawclawclaw::providers::traits::ProviderCapabilities;
